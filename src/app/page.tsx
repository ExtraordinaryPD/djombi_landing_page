...
export default function Home() {
  ...
  // Handle card click - in a real app, this would toggle the feature or navigate
  const handleFeatureClick = (link: string, isActive: boolean) => {
    if (isActive) {
      window.location.href = link;
    } else {
      // Maybe show a coming soon modal or notification
      console.log("Feature coming soon:", link);
    }
  };
  ...
  return (
    <div className="flex flex-col font-[family-name:var(--font-geist-sans)]">
      ...
      >
        <div className="text-center p-8 max-w-6xl mx-auto w-full">
          ...
          <div className="flex justify-center mb-12 border-b border-gray-200">
            {exampleData.map((tab) => (
              ...
              >
                {tab.label}
                {activeTab === tab.id && (
                  <span className="absolute bottom-0 left-0 w-full h-1 bg-emerald-500 rounded-t-md transform translate-y-1"></span>
                )}
              </button>
            ))}
          </div>
...
//   const activeFeatures = exampleData.find(tab => tab.id === activeTab)?.features || [];
